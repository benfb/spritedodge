...
class CollectibleGem
  ...
  def draw
    ...
    @image.draw_rot(@x, @y, 0, 50 * Math.sin(milliseconds / 133.7))
  end
end
...
class CptnRuby
  ...
  def initialize(window, x, y)
    ...
    @standing, @walk1, @walk2, @jump =
      *Image.load_tiles(window, "img/CptnRuby.png", 50, 50, false)
...
class Map
  ...
  def initialize(window, filename)
    ...
    @tileset = Image.load_tiles(window, "img/tileset.png", 64, 64, true)

    gem_img = Image.new(window, "img/good_ball.png", false)
    @gems = []
...
class Game < Window
  ...
  def initialize
    ...
    @sky = Image.new(self, "img/Space.png", true)
